...
Export active options
    
    export var activeOptions=
        browser: undefined
        target: undefined
...
Expose active options (module var)

        activeOptions.browser = options.browser
        activeOptions.target = options.target
        activeOptions.mainModuleName = options.mainModuleName
...
because the lexer preprocessor can compile marcros and generate code in the fly via 'compiler generate'
        ...
        moduleNode.lexer = new Lexer(module.exports)
...
#### method produceModule(moduleNode:Grammar.Module)
        ...
        if moduleNode.fileInfo.outExtension
